 ...
 * `/themes/` routes to `themes.svn.wordpress.org`. The same six-attempt SVN
 * walk and exception semantics apply to both kinds.
 *
 * Six-attempt SVN walk in the following exact order:
 ...
 * e.g. `27.5` → `27.5.0.0`). On `UnexpectedValueException`, attempts 1-3 are
 * skipped and the walk starts at attempt 4 (trunk).
 *
 ...
 * `'Resolved WP package {slug} from {url}'` (info — covers both plugins and
 * themes; the `url` context key disambiguates via `plugins.svn.*` vs
 * `themes.svn.*`), and `'All 6 SVN attempts returned 404 for {slug}'` (warning).
 *
 * All-404 throws `UnsupportedPackageException`. Premium plugins and unlisted
 * themes have no safe fall-through alternative; only public WP plugins and
 * themes are in scope.
 */
final readonly class WordPressOrgResolver implements SourceProviderInterface
{
    private const int ATTEMPT_COUNT = 6;
    ...
    public function resolve(Package $package, VersionRange $range): Source
    {
        ...
        $this->logger->warning('All 6 SVN attempts returned 404 for {slug}', [
            'slug' => $slug,
        ]);
